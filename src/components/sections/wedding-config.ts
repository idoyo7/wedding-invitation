...
interface EffectsConfig {
  ...
  fallingElements: "none" | "hearts" | "idoyo7s" | "snow" | "sparkles" | "minimal" | "geometric"; // 떨어지는 요소들
...
export const weddingConfig = {
  ...
  // 이펙트 설정
  effects: {
    enabled: true, // 전체 이펙트 활성화
    pageTransition: true, // 페이지 로딩 애니메이션 (부드러운 fade-in)
    scrollAnimation: true, // 스크롤 시 섹션 등장 애니메이션
    ...
    fallingElements: "minimal", // 떨어지는 효과 ("none", "hearts", "snow", "sparkles", "minimal", "geometric")
    galleryAnimation: false, // 갤러리 이미지 애니메이션 (썸네일 로딩 문제로 비활성화)
    buttonAnimation: true, // 버튼 호버 애니메이션
  } as EffectsConfig,
}; 
